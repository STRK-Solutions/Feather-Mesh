...
| P0 | Complete (local) | Added [data-access contract](docs/data_access_contract.md), pinned pure-Rust `parquet 60.0.0`/`tiff 0.10.3` inspectors, fixture generator, expected values, invalid inputs, and local STAC profile schema. | Baseline `cargo test` passed before changes. Fixture generator was run with Python 3.13, PyArrow 25.0.1, Rasterio 1.5.1, and NumPy 2.5.3. Target-HPC inspector evidence remains P8. |
| P1 | Complete (local) | Added `mesh_core::services::peer_access`: versioned project config, provider-authorized owner teams, typed manifests/assets/descriptors/errors, project-route checks, fixed references, SHA-256, and copy preflight. Legacy `serve` now anchors new relative records at publication time. | `cargo test -p mesh_core --test peer_access_tests` covers provider/client alias routes, cache loss, unregistered membership, removed peer route, output/source alias, and receipt-write recovery. Broken/cyclic filesystem cases remain covered by route checks; target-mount behavior is pending. |
| P2 | Complete (local) | Provider `manifest.json` is authoritative; publication validates required metadata, Parquet footers/schema, GeoTIFF decoder/geokeys/transform, fixed inventory, digest/size, immutable versions, atomic same-directory writes, lock timeout, and withdrawal tombstones. | Core and CLI tests reject renamed CSV-as-Parquet and leave no manifest; valid raster/table records publish and withdrawal blocks resolution. The target-filesystem lock/rename concurrency exercise remains pending in [HPC checklist](docs/data_access_hpc_checklist.md). |
| P3 | Complete (local) | Added direct configured-route refresh/cache snapshot, current-manifest resolver, path/integrity checks, lifecycle/availability errors, and no filesystem-glob discovery. Cache is per-process/host-local by contract and direct resolution rebuilds from manifests. | Core tests remove cache then resolve successfully, and remove a client peer while the saved provider data remains readable to prove new access fails. Multi-node cache evidence remains pending. |
