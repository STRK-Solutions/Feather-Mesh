...
For the initial HPC use case, the endpoint runs alongside the user's notebook/job and returns assets accessible in that same filesystem context. It uses the same namespace restrictions as the SDK, has no application authentication, and binds only to the exact IPv4 loopback address `127.0.0.1`. The accepted deployment assumption is a small trusted HPC environment: any process able to connect on that node may read the catalog metadata, while operating-system permissions still decide whether it can open an asset. Another-node access requires a separate transport design rather than widening this listener.
