...
Read [AGENTS.md](../../../AGENTS.md), [requirements](../../../data_access.md), and the [workplan](../../../data_access_implementation_workplan.md). Explicit user instructions take precedence. Confirmed requirements govern; example filenames, nested-link policy, metadata fields, command syntax, SDK layout, and other proposed defaults remain provisional until P0 settles them in `docs/data_access_contract.md`. Read that contract when present and keep detailed schemas, policies, and migration decisions there.

The current Rust implementation uses a SQLite registry. Authoritative peer manifests, a supported Python SDK, and the STAC HTTP adapter are planned. Older central-registry/copy-first plans do not govern this feature. Direct peer reads are first-class; staging is explicit.

## Shared Invariants

- Publish one authoritative registered record with an exact versioned asset inventory. Validate before atomic publication; readers see a complete old or new record. SQLite and STAC are rebuildable projections, never independently editable authorities for this workflow. Failure before commit leaves no visible partial version; failure after commit must distinguish committed publication from projection failure and support safe retry/recovery.
...
| STAC HTTP | Use a real standard client such as `pystac-client` against the running server for authenticated collection browsing, spatial/time search, empty results, and multiple pages. Test invalid/missing credentials, invalid queries/cursors, withdrawal/removed peers between pages, and identity/URI round trips into SDK/Rasterio. Exercise every declared conformance class. |
