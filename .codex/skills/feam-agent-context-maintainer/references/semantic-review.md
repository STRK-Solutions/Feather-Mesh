...
- Require installed-package/subprocess evidence for SDK changes, native `LazyFrame` execution over fixed membership, known Rasterio window values, pinned STAC schemas plus real tokenless `127.0.0.1` paginated client tests and rejected non-loopback binding, and documented inspector feature configurations. Commands enter guidance/CI as each component arrives. Missing dependencies/tests and actual HPC evidence must remain explicit; local mocks cannot complete multi-node acceptance.
