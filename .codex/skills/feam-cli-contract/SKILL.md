...
- Keep ordinary `serve` as publication. The implemented `stac serve` starts tokenless HTTP restricted in core to exact IPv4 loopback (`127.0.0.1`). Keep the `feam` user-facing name and configurable `mesh_cli` executable path distinct.
