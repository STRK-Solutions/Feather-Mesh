...
Use MAC for day-to-day coding and native Linux for release artifacts; record dependencies and digests. Reuse an existing binary only when its inputs still match. The U.03 native collector release passed, and main's guided tutorial/UI changes have since merged locally; build and deploy an exact updated native release before claiming those additions on Ubuntu. Full-system VM, reboot and destructive mount tests remain available for later hardening.
...
- [x] Four admins and seven regular users are specified privately after the owner's roster addition; enrollment stays invited. Admins have management access, not automatically allocated workspaces. Target capacity remains ten regular users.
...
- [x] Owner switched the demo back to `613202690.xyz` on 2026-09-25. The original four Cloudflare routes, HTTPS, Tunnel, Access email PIN and authenticated browser/WebSocket checks passed under U.05; the approved expansion now has nine routes; use `https://feam.613202690.xyz` with the sibling routes below. [Public activation](evaluations/web-demo/u05-public-activation.json) and [browser acceptance](evaluations/web-demo/u05-browser-acceptance.json) record the measured scope.
...
- [x] Research traces and reviewed exports survive ordinary shutdown in protected Ubuntu storage. Before destructive teardown, copy the retained corpus and deletion/provenance ledger to private Mac storage and verify hashes. Keep 30-day retention and restricted review. R2 is shelved; U.03 now has deployed local archive, ordinary stop and independently verified private Mac-copy evidence. No destructive teardown has been authorized.

These checked items record settled decisions; implemented and deployed claims are linked to their measured evidence above. The [Stage-1 acceptance record](tui_agent_stage1_acceptance.md#final-fresh-acceptance) contains historical 97/100 held-out model evidence. U.G requires a small functional live smoke; the new held-out benchmark and deployed capacity/reboot measurements are deferred.
...
- [x] U.03 [MAC → UBUNTU] **Remove the runtime R2 dependency.** Explicit production local archive mode uses a fixed private, bounded path on the verified traces filesystem; collector startup, settings validation, service configuration and lifecycle checks agree. Redaction, request/event correlation, quotas, 30-day retention, deletion lineage and reviewer restrictions remain enforced. An actual Ubuntu fake run produced two completed zero-cost requests plus one intentionally canceled unknown-cost request; ordinary stop flushed eight archive objects and retained the ledger. Four non-reviewer categories were denied archive/database/export access. An owner-only Mac copy independently verified all eight object hashes, inventory and provenance ledger; destructive teardown was not authorized. Local tests cover withdrawn/expired retained-copy maintenance. The earlier W1 runtime interruption is retained, and the same container was recovered without reset. [Deployed capture, stop and copy](evaluations/web-demo/u03-fake-capture-archive.json), [W1 recovery](evaluations/web-demo/u03-w1-runtime-recovery.json), [local mode and maintenance tests](evaluations/web-demo/u03-collector-local-archive.md). R2 remains shelved.
...
| W3 | Short Ubuntu lifecycle and ownership/isolation checks pass | Ten-user limits/load and broader recovery remain deferred | Retain measured two-user scope. |
...
| W6 | Local archive and both synthetic/live capture with private Mac copies pass | Original cloud-archive gate remains deferred | Preserve unknown fake cancellation and copy retention lineage. |
...
| W8 | Real Access participant flow, historical admin revocation and bounded live model smoke pass | Fresh admin walkthrough unavailable; benchmark deferred | Keep current route/cap; no provider fallback. |
...
| W10 | Operator handoff and protected archive copies delivered | Full export/recreation/cloud gate deferred | Fresh admin acceptance stays explicit; invitations remain unauthorized. |
...
- The explicit local collector archive and root configuration/lifecycle support are implemented and pass local/native checks. Deployed capture/flush and independently verified Mac-copy evidence now cover both synthetic and live runs. No R2 transfer, R2 Terraform apply or live S3 check is active work.
